...
It enables interaction with the Google Gemini API to execute user tasks, manage conversational sessions,
dynamically load and execute 'skills' (tools), and facilitate the generation of new skill manifests.
The application supports two primary modes: a command-line interface (`run-task`, `generate-skill`)
and an interactive chat loop for continuous interaction.

Key Responsibilities:
--------------------
- **Task Execution**: Drives the core conversational loop with the Gemini LLM for user task execution.
...
# Define the default Generative AI model to be used.
model: Model = Model.gemini_2_5_flash


def run_gemini_task(session: Session, user_task: str) -> str:
    """
    Orchestrates the interaction with the Google Gemini model to execute a user task.
    ...
    It iteratively sends requests to the Gemini model and processes its responses
    ...
    # Create a new chat session with the Gemini client and system instructions.
    session.create_chat(client=client, system_instructions=request.system_prompt, model=model)
    ...
    while True:
        ...
        try:
            # Send the message parts to the Gemini model and get a raw response.
            ...
            if response.needs_approval:
                ...
                if user_approval.lower() != 'yes':
                    ...
                    return session.last_message() # Return the last message on disapproval
...
def generate_skill(skill_name: str, metadata: dict):
    """
    Generates a new skill manifest using the skill_creator agent.

    This function sends a request to the Gemini model, guided by the `skill_creator` agent,
    ...
    # The Gemini model's response will contain the JSON structure for the skill.
...
if __name__ == '__main__':
    ...
    if args.command == 'run-task':
        ...
        # A new session is created or an existing one found, allowing for persistent context.
        session = Session.find_or_create(is_new=True, session_id=None) # Assuming session_id can be passed or generated.
        logger.info(f'Starting run-task for user_task: {args.user_task}')
        ...
        logger.info(f'Starting generate-skill for skill: {args.skill_name}')
        ...
        # This allows continuous interaction with the agent.
